...
summary(filter(zip_pm, date == "2012-07-01"))


pm_range <- zip_pm %>% group_by(date) %>% 
  summarise(geo_min = log(min(geo_smk_pm, na.rm = T)+0.5), 
            geo_med = log(median(geo_smk_pm, na.rm = T)+0.5), 
            geo_max = log(max(geo_smk_pm, na.rm = T)+0.5))
  
summary(pm_range)

ggplot(pm_range, aes(x = date)) +
  geom_ribbon(aes(ymin = geo_min, ymax = geo_max), fill = "grey70") +
  geom_line(aes(y = geo_med)) +
  ylab(expression('ln Population-Weighted PM2.5 µg/m'^3)) +
  xlab('Date') + 
  theme(panel.background = element_rect(fill = 'white', colour = 'black'),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
