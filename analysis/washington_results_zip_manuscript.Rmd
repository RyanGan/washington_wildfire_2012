...
#### Region-Specific Smoke PM~2.5~ Timeseries 

Figures below are for each smoke PM~2.5~ estimation method. The figures are the daily range (min and max) for each zip code in a specific region of the state. The figure first starts out by showing the map of the state, divided in to eco regions used by the state for organization purposes. 
...
# I can loop plots though

# create plot for WRF-Chem
wrf_plot <- ggplot(wrf_range, aes(x = date)) +
  geom_ribbon(aes(ymin = min, ymax = max, group = eco_region, fill=eco_region), alpha =0.5) +
...
plot_grid(eco_region_map, wrf_plot, align = "h")
