...
summary(filter(zip_pm, date == "2012-07-01")) # quick sum of first zip obs 

# wrf smoke
wrf_range <- zip_pm %>% group_by(date) %>% 
  summarise(min = log(min(wrf_smk_pm, na.rm = T)+0.5), 
            med = log(median(wrf_smk_pm, na.rm = T)+0.5), 
            max = log(max(wrf_smk_pm, na.rm = T)+0.5)) %>% 
  mutate(est_method = "wrf_smk")

# krig smoke
krig_range <- zip_pm %>% group_by(date) %>% 
  summarise(min = log(min(krig_smk_pm, na.rm = T)+0.5), 
            med = log(median(krig_smk_pm, na.rm = T)+0.5), 
            max = log(max(krig_smk_pm, na.rm = T)+0.5)) %>% 
  mutate(est_method = "krig_smk")

# global smoke
global_range <- zip_pm %>% group_by(date) %>% 
  summarise(min = log(min(global_smk_pm, na.rm = T)+0.5), 
            med = log(median(global_smk_pm, na.rm = T)+0.5), 
            max = log(max(global_smk_pm, na.rm = T)+0.5)) %>% 
  mutate(est_method = "global_smk")

# geo smoke
geo_range <- zip_pm %>% group_by(date) %>% 
  summarise(min = log(min(geo_smk_pm, na.rm = T)+0.5), 
            med = log(median(geo_smk_pm, na.rm = T)+0.5), 
            max = log(max(geo_smk_pm, na.rm = T)+0.5)) %>% 
