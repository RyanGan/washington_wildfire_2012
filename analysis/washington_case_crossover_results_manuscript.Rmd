...
### Wenatchee zip code time series
```{r zip 98801, echo = F, results='asis'}

zip_98801 <- zip_pm %>% filter(ZIPCODE == "98801")

# time series plot
zip_pm_times_series <- ggplot(zip_98801, aes(x = date)) + 
  #scale_x_date(date_labels = '%m')+ # having issues with the date scale
  geom_jitter(aes(y = wrf_pm, colour = 'WRF-Chem'), size = 0.75) + 
  geom_jitter(aes(y = geo_wt_pm, colour = 'Geo-Weight'), size = 0.75) +
  # custom colour
...
shp_dir <- paste0('/Users/ryangan/Google Drive/CSU/wild_fire/shape_files/',
...
###Note: Suppressing the code and output for this chunk for now 
...
```{r time stratified design county within fire season, eval = F, echo = F, results='asis'} 
# note above, results = 'asis' needed to print htmlTables
