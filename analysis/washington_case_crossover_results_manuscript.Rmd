...
# call terrain map
wash_map_gterrain <- get_map(location = bbox, source = 'google',
...
# county smoke days by washington county
proj4string(wash_zip_map)
# set coord ref system to WGS84
wash_zip_map <- spTransform(wash_zip_map, "+proj=longlat +datum=WGS84")


ggmap(washington_map) + 
  geom_polygon(data = wash_zip_map, aes(x=long, y=lat, group = group), fill = NA, colour = 'red')
...
I'd like to look at some outcomes stratified by age category. Note that kids <=15 likely won't have many outcomes like COPD or MI.

```{r time stratified design within fire season age, echo = F, results='asis'} 
# note above, results = 'asis' needed to print htmlTables
