...
More referent periods is better for statistical efficiency, but there is a tradeoff, where additional referent periods may introduce what they call 'overlap bias', which they mathmatically define in Janes 2005 Stats in Medicine paper, *Overlap Bias in the Case-Crossover Design, with Applications to Air Pollution Exposures*. Basically this bias occurs when the exposure assigned in the referent period is conditional on the strategy used to define the referent period.

As Janes et al. state in their 2005 Stats in Med paper:
...
Similar to the last seasonal example, this time-stratified referent strategy allows even more referent observations to average to 'smooth' out some of those extreme PM~2.5~ concentrations in some referent periods.s
...
## Case-Crossover Results: Symmetric Bi-Directional (14, 21, 28 day referent periods)

Another popular design is the symmetric bi-directional approach, as symmetry of counterfactual observations included before and after reduces bias (Schwartz 2001), I only include symmetric referent observations. For example, if  subject has 4 observations before the event with PM~2.5~ values and only 1 observation after the event with PM~2.5~ values. I excluded the first value after as I think this may introduce overlap bias as the distribution of PM~2.5~ could be relativley high.

**Note: 1 observation period = 7 days**Referent Selection Strategy: Same Day of Week Selected Before and After


```{r symmetrical and bi-directional obs 2 3 4, echo = F, results='asis'} 
# note above, results = 'asis' needed to print htmlTables
...
for(i in 1:length(df_list)){
  ...
  df_analysis <- cbind(covariates_df, pm_estimates_df) %>% 
    ...
    mutate(min_obs_diff = abs(min(obs_diff_admission)),
           ...
           keep_obs2 = ifelse(obs_diff_admission == 0 | 
                              #obs_diff_admission == 1 | obs_diff_admission == -1 | 
                              obs_diff_admission == 2 | obs_diff_admission == -2 |
                              obs_diff_admission == 3 | obs_diff_admission == -3 |
                              obs_diff_admission == 4 | obs_diff_admission == -4, 
                              1, 0),
           # get rid of people with single observations
           keep_obs3 = ifelse(obs_limit >= 2, 1, 0)) %>% 
...
table_df <- combined_point_est_df %>% select(2, 3:7) 
  ...
  tab <- htmlTable(txtRound(table_df, digits = 3, 1:3), 
           ...
           css.cell = "padding-left: 0.5em; padding-right: .5em;", # cell space
           align = "llccccc", # column alignment,
           tfoot="&dagger; Adjusted for temperature, accounting for subject. Symmetric bi-direction 14, 21, and 28 days before and after."
            ) # end table
