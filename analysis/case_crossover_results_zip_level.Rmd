...
# combine each outcome itteration in to a big dataset
combined_point_est_df <- bind_rows(datalist)

glimpse(combined_point_est_df)
table_df <- cbind(data.frame(method_list), point_est_df[, c(1, 3:7)]) %>% 
    select(2, 1, 3:7) # rearrange to make the outcome the first variable
  ...
  tab <- htmlTable(txtRound(table_df, digits = 3, 1:3), 
           ...
           #n.rgroup = 3,
           # column headers
           header = c("Outcome", "Method", "Obs.", "Events",
