...
```{r interaction with pm and smoke, eval = F, warning= F,}
# use the wash_jul_oct df to test interaction in the poisson model 

model_adj <- glmer(asthma_n_wo_miss ~ geo_wt_pm_unit10 + geo_smk10 + 
                 geo_wt_pm_unit10*geo_smk10 + wrf_temp + (1|county), 
                 wash_jul_oct_df, family='poisson')
summary(model_adj)
...
for(i in 1:length(df_list)){
  ...
  #which(colnames(df_to_loop)=="geo_wt_pm_county") 
  ...
  df_analysis <- cbind(covariates_df, pm_estimates_df) %>% 
    ...
    mutate(obs_diff_admission = (date - date_admit)/7) %>%
      # create a binary smoke indicator variable based smk variables >5, >10, and >15 units 
    mutate(wrf_smk5 = ifelse(wrf_smk_pm_county >= 5, 1, 0), 
           wrf_smk10 = ifelse(wrf_smk_pm_county >= 10, 1, 0),
           wrf_smk15 = ifelse(wrf_smk_pm_county >= 15, 1, 0), 
           geo_smk5 = ifelse(geo_smk_pm_county >= 5, 1, 0),
           geo_smk10 = ifelse(geo_smk_pm_county >= 10, 1, 0),
           geo_smk15 = ifelse(geo_smk_pm_county >= 15, 1, 0),
           krig_smk5 = ifelse(krig_smk_pm_county >= 5, 1, 0),
           krig_smk10 = ifelse(krig_smk_pm_county >= 10, 1, 0),
           krig_smk15 = ifelse(krig_smk_pm_county >= 15, 1, 0)) %>% 
    # group by patient id to do some within subject data manipulation
    group_by(PATIENTID) %>% 
