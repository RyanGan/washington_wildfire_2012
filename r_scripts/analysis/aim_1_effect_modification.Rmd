...
effect_table <- matrix(data = NA, nrow = 4, ncol = 5) %>% 
  as_tibble() %>% 
  rename(estimate = V1, 
         odds_ratio = V2,
         lower95 = V3,
...
effect_table[,1] <- c("pm_10unit", "smoke", "pm_diff","pm_smoke_10unit") 
# populate table 
effect_table[1,2:4] <- round(exp(mod_tidy[1, c(2,6:7)]),2) # OR and 95CI
effect_table[1,5] <- round(mod_tidy[1,5],2) # pval
effect_table[2,2:4] <- round(exp(mod_tidy[2, c(2,6:7)]),2) # OR and 95CI
effect_table[2,5] <- round(mod_tidy[2,5],2) # pval
# test for interaction
effect_table[3,2:4] <- round(exp(mod_tidy[4, c(2,6:7)]),2) # OR and 95CI
effect_table[3,5] <- round(mod_tidy[4,5],2) # pval
...
p <- 2*pnorm(-abs(z))
# fill in table
effect_table[4, 2:4] <- round(exp(c(est, l95, u95)),2)
# p val
effect_table[4,5] <- round(p,2)
...
sim_data <- as_tibble(rbinom(n, 1, prob = 0.08)) %>% 
  rename(gwr_smk5)
mean(sim_data$value)
mean(pm$gwr_smk5)
mean(pm$geo_wt_pm)
