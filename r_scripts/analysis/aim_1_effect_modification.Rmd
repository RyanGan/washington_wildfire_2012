...
mod1 <- clogit(outcome ~ geo_wt_pm_zip + gwr_smk10 + gwr_smk10*geo_wt_pm_zip +
                wrf_temp_zip + strata(PATIENTID), data =asthma)
mod2 <- broom::tidy(mod1)
mod2
...
mod <- clogit(outcome ~ geo_wt_pm_zip + gwr_smk0 + gwr_smk0*geo_wt_pm_zip +
