...
for(i in 1:length(df_list)){
  ...
  # bind with CDC county-level met variables provided by rish
    left_join(county_met2, by = c("county", "date"))
  ...
  # second loop to run a model for each pm estimation method
    for(j in 55:58){
      ...
      mod <- clogit(outcome ~ df_analysis[[j]] + daily_meanRH + daily_meanT +
                    daily_meanWS + daily_meanPrcp + 
