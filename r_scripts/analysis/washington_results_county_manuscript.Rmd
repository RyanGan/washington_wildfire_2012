...
#  which(colnames(df_to_loop)=="day") # code to find column numbers
    
  # dataframe for analysis creation
  # bind columns back together 
  df_analysis <- df_to_loop %>% select(c(1:16, 87, 95:96, 152:158)) %>% 
...
for(i in 1:length(df_list)){
  ...
  covariates_df <- df_to_loop[, c(1:16, 95:96, 152:158)] 
  
  # modified to select county estimates
  # extract pm values and divide by 10 and ordered by how I want to present in paper
  #which(colnames(df_to_loop)=="geo_smk_pm_county") # code to find column numbers
  pm_estimates_df <- df_to_loop[, c(87, 94, 93, 92)]/10  # create 10 unit increases
  ...
  #which(colnames(df_analysis)=="wrf_smk_pm_county")
...
daily_county_met <- county_met %>%
  ...
  mutate(cdc_temp_lag1 = lag(daily_meanT, 1, order_by = county),
         ...
         cdc_temp_lag4 = lag(daily_meanT, 4, order_by = county),
         cdc_temp_lag5 = lag(daily_meanT, 5, order_by = county),
         ...
         cdc_rh_lag4 = lag(daily_meanRH, 4, order_by = county),
         cdc_rh_lag5 = lag(daily_meanRH, 5, order_by = county),         
         ...
         cdc_ws_lag4 = lag(daily_meanWS, 4, order_by = county),
         cdc_ws_lag5 = lag(daily_meanWS, 5, order_by = county),
         ...
         cdc_prcp_lag4 = lag(daily_meanPrcp, 4, order_by = county),
         cdc_prcp_lag5 = lag(daily_meanPrcp, 5, order_by = county)) 
...
These results also adjust for the CDC variables, which will be the most comparable to Rish's mortality estimates.
...
# dataframe to loop through
  ...
  pm_estimates_df <- df_to_loop[, c(87, 107:111, 94, 142:146,
                                    93, 137:141, 92, 132:136)]/10  # create 10 unit increases
  ...
  df_analysis <- cbind(covariates_df, pm_estimates_df) %>% 
    ...
    mutate(obs_diff_admission = (date - date_admit)/7,
           ...
           cdc_temp_avg = (daily_meanT + cdc_temp_lag1 + cdc_temp_lag2)/3,
           cdc_rh_avg = (daily_meanRH + cdc_rh_lag1 + cdc_rh_lag2)/3,
           cdc_ws_avg = (daily_meanWS + cdc_ws_lag1 + cdc_ws_lag2)/3,
           cdc_prcp_avg = (daily_meanPrcp + cdc_prcp_lag1 + cdc_prcp_lag2)/3)
  ...
  # second loop to run a model for each pm estimation method
    for(j in 79:82){
      ...
      # conditional logistic regression model
      mod <- clogit(outcome ~ df_analysis[[j]] + cdc_temp_avg + cdc_rh_avg +
                    cdc_ws_avg + cdc_prcp_avg +
                    strata(PATIENTID), df_analysis)
...
table_df <- combined_point_est_df %>% select(2, 3:7) 
  ...
  tab <- htmlTable(txtRound(table_df, digits = 3, 1:3), 
           ...
           css.cell = "padding-left: 0.5em; padding-right: .5em;", # cell space
           align = "llccccc", # column alignment,
           tfoot="&dagger; Adjusted for CDC temperature, relatively humidity, wind speed, and precipitation. Accounting for subject. Time-stratified: referent periods matched to events on same day of week within July to October fire season. Smoke predictor is a moving lag average from 0 to 2 days."
            ) # end table
...
for(i in 1:length(df_list)){
  ...
  # dataframe to loop through
  df_to_loop <- data.frame(df_list[i]) %>% 
    left_join(daily_county_met, by = c("county", "date"))
  ...
  pm_lag_list <- list(df_to_loop[, c(87, 107:111)]/10, 
                      df_to_loop[, c(94, 142:146)]/10,
                      df_to_loop[, c(93, 137:141)]/10, 
                      df_to_loop[, c(92, 132:136)]/10)  # create 10 unit increases
  
  
  # which(colnames(df_to_loop)=="cdc_prcp_lag5") # code to find column numbers
  
  cdc_lag_vars <- df_to_loop[, c(162, 175:179, 161, 180:184, 
                                 164, 185:189, 163, 190:194)]
  ...
  for(k in 1:4){
    ...
    # fill in the outcome name dataframe before method loop
    point_estimates[, 1] <- outcome_name
    # repeat 4 times for each pm method
  ...
  # loop to run a model for each lag 0-5
    for(j in 0:5){
      ...
      mod <- clogit(outcome ~ df_analysis[[pm_lag_colnum]] + df_analysis[[temp_lag_colnum]] +
                      df_analysis[[rh_lag_colnum]] + df_analysis[[ws_lag_colnum]] +
                      df_analysis[[prcp_lag_colnum]] + strata(PATIENTID), df_analysis)
...
## ggplot
  print_plot <- ggplot(geo_smk_lag_plot,
    ...
    facet_wrap(~outcome, nrow = 3) +
    geom_hline(yintercept = 1, linetype=2) +
    ggtitle('Association Between PM2.5 from Wildfire Smoke on Hospitalizations') +
    ylab('Odds Ratio for 10µg/m^3 Increase in PM2.5') +
    #ylim(0.5, 1.5) +
