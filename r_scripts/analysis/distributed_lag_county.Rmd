...
=======
I've tried a mixed effects model and it doesn't coverge. I will need to do a two-stage model. Gasparrini and Armstrong published an extension of their original distributed lag model work that is a two-stage approach that can pool the results from each region. Perfect for this study.
>>>>>>> a265e7d3ae524330ac7bcd2a9ac110a5065ebcd5

I'm following the code from Gasparrini's two-stage paper to run distributed lag models for each county and then combine estimates via meta-analysis.
...
suppressWarnings(
cb <- crossbasis(county_list_df[[county_name]]$geo_smk_pm_unit10,
...
# run the model
model <- glm(asthma_n_wo_miss ~ cb + ns(date, df=3),
...
pred_smk <- crosspred(cb, model, at = 0:20, bylag = 0.2, cumul = T)
pred_smk$matRRfit
...
```{r first stage county prediction}
#  these first two commands aren't necissary since I'm assuming linear for now
bound <- c(max(smk_range[,1]), max(smk_range[,2]))
varknots <- bound[1] + diff(bound)/3*(1:2) # I actually don't need this
...
yall <- matrix(NA, length(county_list_df), 4, 
               dimnames=list(counties,paste("b",seq(4),sep="")))

# (CO)VARIANCE MATRICES
Sall <- vector("list",length(county_list_df))
names(Sall) <- counties
...
for(i in seq(county_list_df)) {
  ...
  suppressWarnings({
    cb <- crossbasis(sub$geo_smk_pm_unit10,
  ...
  # adding in a smoothed spline causes some models not to converge
...
# ----
  ...
  suppressWarnings({
    crall <- crossreduce(cb, model, type = "var", value = 1)
...
county_all <- apply(yall,1,function(x) exp(blag%*%x))
...
cp_smk10 <- crosspred(blag, coef=coef(mvall),vcov=vcov(mvall),
  model.link="log", at = 0:50/5, bylag = 0.2)

cp_smk10$predvar

# START HERE ----
# plot
plot(cp_smk10, type="n", ylab="RR",ylim=c(0.8,1.4),xlab="Lag")
matplot(xlag, cp_smk10, type="l",col=grey(0.5),lty=2,add=T)
