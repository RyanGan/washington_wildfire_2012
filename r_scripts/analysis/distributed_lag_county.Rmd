...
wa_state_aggregate_jul_oct <- wa_state_aggregate %>% 
  filter(date >= "2012-07-01" & date <= "2012-10-31") %>% 
...
# using aggregated asthma count dataset
# define the function of the spline for the asthma trend for the entrie state
spl <- ns(asthma_counts$date, df = 3)

# model n asthma events with a spline with 7 df (knotches)
model1 <- glm(n ~ spl, asthma_counts, family="quasipoisson") 
summary(model1)

pred1 <- predict(model1, type="response")

plot(asthma_counts$date, asthma_counts$n,
     ylim=c(0, 30),pch=19,cex=0.2,col=grey(0.1),
     main="Flexible cubic spline model",ylab="Daily Number of Asthma ER/Urgent",
     xlab="Date")
lines(asthma_counts$date, pred1, lwd=2)

# plot residuals
res1 <- residuals(model1,type="response")

plot(asthma_counts$date, res1,ylim=c(-15,15),pch=19,cex=0.4,col=grey(0.6),
